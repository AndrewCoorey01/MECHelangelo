// =============================================================================
// MECHelangelo Behaviour Node — Version 4a (timed-turn calibration attempt)
//
// An experimental iteration built on top of v4. It was tested on the physical
// base but reverted to v4 because the split turn rates did not improve tracking.
//
// Key differences from v4 (current):
//   - Split timed-turn rates: kAutonomousTimedTurnRatePositive=0.30 rad/s (left),
//     kAutonomousTimedTurnRateNegative=0.27 rad/s (right) — physical asymmetry
//   - kAutonomousWallConfirmationScans=2 — requires wall to appear in 2 consecutive
//     scans before committing to a stop (reduces false stops from single-frame noise)
//   - kAutonomousPostTurnSettleSeconds=0.40 s settle after each turn
//   - #include <cstdint> added (was missing in v4)
//
// Reverted because split turn rates required re-calibration and the wall
// confirmation delay caused the robot to occasionally overshoot tight corners.
// =============================================================================
...
// Physical autonomous movement tuning only. Human tracking uses its own
// constants below and is intentionally unaffected by these values.
...
// A direction is open only when it contains a genuine raw no-return (+inf)
// or a retained finite filtered return beyond this clearance. Finite points
// rejected by the heavy filter are stored as NaN (unknown), never as open.
static constexpr double kDvdOpenClearanceDistance = 2.25; // m

// Require a broad opening and keep the selected heading away from its edges.
static constexpr double kDvdMinSectorWidth = 24.0 * M_PI / 180.0; // radians
static constexpr double kDvdSectorEdgeMargin = 10.0 * M_PI / 180.0; // radians
...
// dots are usually isolated or only one/two points, so they become NaN
// (unknown) and cannot be selected as open space or stop the robot.

// Stage 1: stricter local neighbour test for the noisy physical LiDAR.
static constexpr int kNoiseNeighbourWindow = 5;          // check +/- 5 beams
static constexpr int kNoiseMinNeighbourCount = 4;        // require a locally dense return
static constexpr double kNoiseNeighbourDistance = 0.18;  // m in local XY space

// Stage 2: retain only coherent wall-sized segments.
static constexpr double kSegmentJoinDistance = 0.16;     // m max gap between consecutive points
static constexpr int kSegmentMinPoints = 7;              // reject short coherent speckle clusters
static constexpr double kSegmentMinLength = 0.12;        // m reject tiny objects/noise

// ------------------------------------------------------
// Physical autonomous-only runtime state
// ------------------------------------------------------
...
static int g_autonomous_blocked_scan_count = 0;
static std::uint64_t g_filtered_scan_generation = 0;
static std::uint64_t g_autonomous_last_evaluated_scan_generation = 0;

// Last heavy-filter accounting. Rejected finite returns remain unknown (NaN).
static int g_filter_last_input_finite_points = 0;
static int g_filter_last_kept_finite_points = 0;
static int g_filter_last_rejected_finite_points = 0;
static int g_filter_last_raw_open_points = 0;
    ...
    case NavigationState::MOVING:
    {
        ...
        if (blocked_by_segment || front_range <= stop_distance_m_)
        {
            if (blocking_segments.empty())
            {
                ...
                fallback.midpoint.x = std::isfinite(front_range)
                    ? front_range
                    : stop_distance_m_;
...
void MechelangeloBehaviour::imuCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
    latest_imu_ = *msg;
    imu_available_ = true;
    g_autonomous_imu_message_received = true;
    g_autonomous_last_imu_time = this->now();
}
    ...
    const sensor_msgs::msg::LaserScan &raw_scan)
{
    ...
    const float open_value =
        std::numeric_limits<float>::infinity();
    const int scan_count =
        static_cast<int>(raw_scan.ranges.size());

    std::vector<double> x_points(
        scan_count, std::numeric_limits<double>::quiet_NaN());
    std::vector<double> y_points(
        scan_count, std::numeric_limits<double>::quiet_NaN());
    std::vector<bool> usable(scan_count, false);
    std::vector<bool> raw_open(scan_count, false);
    std::vector<bool> input_finite(scan_count, false);

    g_filter_last_input_finite_points = 0;
    g_filter_last_kept_finite_points = 0;
    g_filter_last_rejected_finite_points = 0;
    g_filter_last_raw_open_points = 0;

    // Preserve a genuine positive infinity from the raw driver as no-return.
    // Any finite point removed by the heavy filter becomes NaN (unknown), so
    // heading selection can never mistake rejected noise for open space.
    for (int i = 0; i < scan_count; ++i)
    {
        ...
        {
            raw_open[i] = true;
            neighbour_filtered.ranges[i] = open_value;
            final_filtered.ranges[i] = open_value;
        ...
        {
            neighbour_filtered.ranges[i] = unknown_value;
            final_filtered.ranges[i] = unknown_value;
            continue;
        }

        input_finite[i] = true;
        g_filter_last_input_finite_points++;
    ...
    // Stage 1: suppress points without a dense local neighbourhood.
    for (int i = 0; i < scan_count; ++i)
    {
        if (!usable[i])
        {
        ...
        const int start_index =
            std::max(0, i - kNoiseNeighbourWindow);
        const int end_index =
            std::min(scan_count - 1, i + kNoiseNeighbourWindow);
    ...
    // Stage 2: retain only coherent wall-sized segments.
    const std::vector<LaserSegment> accepted_segments =
        buildLaserSegments(neighbour_filtered);

    std::fill(
        final_filtered.ranges.begin(),
        final_filtered.ranges.end(),
        unknown_value);

    // Restore genuine raw no-return beams after the fill.
    for (int i = 0; i < scan_count; ++i)
    {
        if (raw_open[i])
        {
    ...
    {
        for (int i = segment.start_index;
             i <= segment.end_index;
             ++i)
        {
            if (i >= 0 && i < scan_count &&
                std::isfinite(neighbour_filtered.ranges[i]))
            {
                final_filtered.ranges[i] =
                    neighbour_filtered.ranges[i];
            }
        }
    }
    ...
    for (int i = 0; i < scan_count; ++i)
    {
        ...
        if (std::isfinite(final_filtered.ranges[i]))
        {
...
bool MechelangeloBehaviour::getLongestRange(double &out_angle, double &out_range) const
{
    ...
    //
    // Open means either:
    //   - positive infinity preserved directly from the raw LiDAR, or
    //   - a retained finite wall return farther than the clearance distance.
    // NaN means the heavy filter rejected the point and is always unknown.
    // ------------------------------------------------------
    ...
    if (selected_index >= 0)
    {
...

        {
            ...
            log << "Reposition commits: " << g_human_reposition_commit_count << "\n";
            log << "Settle successes: " << g_human_settle_success_count << "\n\n";
            log << "Open beam rule: raw +inf OR retained finite range >= "
                << kDvdOpenClearanceDistance << " m; filtered rejects are NaN/unknown\n";
            log << "Filter input finite points: " << g_filter_last_input_finite_points << "\n";
            log << "Filter kept finite points: " << g_filter_last_kept_finite_points << "\n";
            log << "Filter rejected finite points: " << g_filter_last_rejected_finite_points << "\n";
            log << "Raw no-return points: " << g_filter_last_raw_open_points << "\n";
