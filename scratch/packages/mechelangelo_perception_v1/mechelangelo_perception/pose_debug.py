...
#
# WHAT THIS FILE DOES:
#   Standalone (non-ROS) pose debugging script for the Raspberry Pi.
#   Used during early development to verify that MediaPipe, the Pi camera,
#   and MQTT publishing were all working before integrating with ROS.
#
#   Unlike pose_tracking_human_ros.py this script:
#     - Has NO ROS integration (no nodes, no topics)
#     - Runs directly on the Pi with Picamera2
#     - Streams annotated video via Flask MJPEG on port 5000
#     - Publishes arm angles directly over MQTT (no lock-on state machine)
#     - Processes every frame (no skip-frame optimisation)
#
#   The MQTT broker IP here (192.168.4.62) may be outdated — the main
#   tracking script uses 192.168.4.65.
#
# HOW TO RUN (directly on the Pi, not via ROS):
#   python3 pose_debug.py
#   Then open http://192.168.4.58:5000 on laptop
#
# FUNCTIONAL DIFFERENCES FROM pose_tracking_human_ros.py:
#   - No lock-on / grace period logic
#   - No human tracking (no /human_tracking topic)
#   - No robot movement commands
#   - Simpler frame loop — always processes every frame
#   - Hardcoded Picamera2 (no --camera argument)
#   - Different MQTT broker IP: 192.168.4.62
#
# SEE ALSO:
#   pose_tracking_human_ros.py — the full ROS-integrated version
# =============================================================================
