...
# """
# MECHelangelo simulation camera human tracking node.

# Purpose:
# - Subscribe to the Gazebo/ROS camera topic from the MECHelangelo SDF model.
# - Run MediaPipe Pose on the simulation camera frames.
# - Lock onto a detected human/person-like target.
# - Estimate centre-frame offset and distance from shoulder width.
# - Publish tracking data to /human_tracking for the C++ behaviour node.
# - Keep the existing debug video stream, MQTT arm angle publishing, and overlays.

# Expected camera topic from the provided SDF:
#     /mechelangelo/camera/image_raw
...
# # ── ROS camera topic from simulation SDF ───────────────────────────
# # SDF camera plugin uses:
# #   <namespace>/mechelangelo</namespace>
# #   <remapping>image_raw:=camera/image_raw</remapping>
# # So the expected resolved topic is:
# CAMERA_TOPIC = "/mechelangelo/camera/image_raw"
# TRACKING_TOPIC = "/human_tracking"

# tracking_node = None

# # ── MQTT ───────────────────────────────────────────────────────────
# # MQTT is kept from your existing code so the arm-angle pipeline can still work.
# # For simulation-only movement testing, MQTT can fail without stopping /human_tracking.
...
# class SimCameraTrackingNode(Node):
#     """
#     ROS 2 node that subscribes to the Gazebo camera image and publishes compact
#     human tracking data for the behaviour node.
#     """

#     def __init__(self):
#         super().__init__("sim_camera_human_tracking")
...
#         self.get_logger().info(f"Subscribed to simulation camera: {CAMERA_TOPIC}")
#         self.get_logger().info(f"Publishing human tracking: {TRACKING_TOPIC}")
...
# # ── Command state, retained mostly for debug overlays/MQTT ──────────
# class RobotCommand:
#     FORWARD = "FORWARD"
#     BACKWARD = "BACKWARD"
#     LEFT = "TURN_LEFT"
...
#         # Different region. Only switch if the new target is clearly better.
...
#         frame = tracking_node.get_latest_frame()

#         if frame is None:
#             now = time.monotonic()
#             if now - last_no_frame_log > 2.0:
#                 print(f"[VISION] Waiting for frames on {CAMERA_TOPIC}")
...
#                     # Centre offset. Negative = left of centre, positive = right.
#                     shoulder_mid_x = (lm[11].x + lm[12].x) / 2.0
#                     centre_offset = shoulder_mid_x - 0.5

#                     robot_cmd.update(shoulder_mid_x)

#                     # Publish ROS tracking output for behaviour controller.
...
#                     # During grace, tell behaviour that vision is not currently valid.
...
#         else:
#             # Off-frame: redraw last labels without rerunning MediaPipe.
...
#         "<h2>MECHelangelo Simulation Pose Stream</h2>"
#         f"<p>Camera topic: {CAMERA_TOPIC}</p>"
#         f"<p>Tracking topic: {TRACKING_TOPIC}</p>"
#         '<img src="/video">'
#         "</body></html>"
#     )
...
#     rclpy.init()
#     tracking_node = SimCameraTrackingNode()
...
#     print(f"Publishing tracking topic: {TRACKING_TOPIC}")
