...
void MechelangeloBehaviour::controlLoop()
{
    ...
    case NavigationState::HUMAN_DETECTED:
    {
        ...
        // is estimated from the commanded velocity, while rotation uses measured
        // IMU yaw whenever available. This keeps the remembered human location at
        // the correct relative bearing while the base follows a wall-clearance arc.
        if (g_human_target_locked)
        {
            ...
            if (imu_available_)
            {
                ...
                tf2::fromMsg(latest_imu_.orientation, target_q);
                const double current_imu_yaw = tf2::getYaw(target_q);
                if (g_human_target_imu_yaw_valid)
                {
                    measured_yaw_delta = normaliseAngle(
                        current_imu_yaw - g_human_target_last_imu_yaw);
                }
                g_human_target_last_imu_yaw = current_imu_yaw;
                g_human_target_imu_yaw_valid = true;
            }
        ...
        else if (imu_available_)
        {
            tf2::Quaternion target_q;
            tf2::fromMsg(latest_imu_.orientation, target_q);
            g_human_target_last_imu_yaw = tf2::getYaw(target_q);
            g_human_target_imu_yaw_valid = true;
        }
