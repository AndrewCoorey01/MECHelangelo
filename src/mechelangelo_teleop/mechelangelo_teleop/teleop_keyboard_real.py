...
# ============================================================
# GPIO PIN CONFIGURATION FOR ORIGINAL 6-PIN MOTOR DRIVER
# ============================================================
...
# Keep this lower for first tests with wheels on the ground.
MAX_DUTY = 0.50

# Minimum PWM needed to overcome motor/gearbox friction.
# If the motor command is non-zero, it will be lifted to at least this value.
MIN_MOVING_DUTY = 0.20
...
# If no key command is received for this long, target speed goes to zero.
COMMAND_TIMEOUT = 2.0

# Open-loop trim.
# If one side is weaker/stronger, adjust these slightly.
LEFT_TRIM = 1.00
RIGHT_TRIM = 1.00


MSG = """
MECHelangelo Physical Wheel Test - 6 Pin Motor Driver
-----------------------------------------------------
...
q : test left motor only
e : test right motor only
...
class Motor:
    ...
    def brake(self):
        """
        Optional braking mode.
        Some H-bridges brake when both direction pins are HIGH.
        Use carefully. For normal testing, stop() is safer.
        """
        self.in1.on()
        self.in2.on()
        self.pwm.value = 0.0
...
def main():
    ...
    # If pressing w makes one wheel spin backwards, change invert=False to True
    # for that motor only.
    left_motor = Motor(
        LEFT_PWM_PIN,
        LEFT_IN1_PIN,
    ...
    try:
        while True:
            ...
            if key == "w":
                ...
                left_motor.set_speed(0.30)
                right_motor.stop()

                last_command_time = time.time()
                print("Testing LEFT motor only at +0.30 PWM command")
                continue
                ...
                right_motor.set_speed(0.30)
                left_motor.stop()

                last_command_time = time.time()
                print("Testing RIGHT motor only at +0.30 PWM command")
                continue
            ...
            # Safety timeout.
            # If no command has been received recently, ramp target back to zero.
            ...
            # Apply open-loop trim.
            left_cmd *= LEFT_TRIM
            right_cmd *= RIGHT_TRIM

            # Limit final commands.
